...
contract CurateMarketplace is ReentrancyGuard {
  ...
  event LogMarketplaceItemCreated (
    uint indexed itemId,
  ...
  function createMarketItem(
    ...
    idToMarketItem[itemId] =  MarketplaceItem(
      itemId,
      nftContract,
      tokenId,
      payable(msg.sender),
    ...
    emit LogMarketplaceItemCreated(
      itemId,
      nftContract,
      tokenId,
      msg.sender,
  ...
  function fetchItemsCreated() public view returns (MarketplaceItem[] memory) {
    uint totalItemCount = _itemIds.current();
    uint itemCount = 0;
    uint currentIndex = 0;
